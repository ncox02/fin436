...
**After:** In the editor, you open the chat sidebar and describe what you want to build — AI proposes structure and logic before a single file exists.

**Write**
**Before:** In a browser, you copy code from chat, switch windows, and paste manually.
**After:** In the editor, ghost text appears inline as you type — press `Tab` to accept, no switching.
...
**Takeaway:** AI reasons about structure before a single file exists. This is the right order of operations — understand the design first, then build it. The session keeps this context available for every exercise that follows.
...
- **Module** — a `.py` file that can be imported by other files
- **Import** — how one Python file uses code defined in another
- **Helper function** — a small function with one job, called by larger functions
- **Type annotation** — `def f(x: float) -> float:` — declaring what types a function expects and returns
- **Virtual environment** — why Python projects isolate their packages
- **pytest fixture** — reusable setup code for tests
- **Vectorization** — why operating on whole arrays is faster than looping row-by-row
- **Docstring** — the standard way to document what a function does
- **Stack trace** — how to read a Python error from bottom to top
