...
## What we just did {#concepts}

Twelve exercises, each using a different AI capability:

- **Exercise 1 — Blueprint prompt:** Copilot Chat planned project structure from a plain-English description — no files open yet
- **Exercise 2 — Scaffold generation:** Agent mode created the folder and file structure directly
- **Exercise 3 — Comment-driven generation:** Ghost text completed a function from a comment spec
- **Exercise 4 — Finish starter code:** Inline edit (`Ctrl+I`) completed a function from its comments
- **Exercise 5 — Cross-file context:** `#filename` references gave the AI simultaneous access to two files mid-conversation
- **Exercise 6 — Explain a block:** Right-click → Explain generated an on-demand explanation of selected code
- **Exercise 7 — Spot a bug:** Copilot Chat reviewed a function for bugs and edge cases in one prompt
- **Exercise 8 — Refactor for performance:** Right-click → Review and Comment identified a performance problem and the vectorized fix
- **Exercise 9 — Terminal crash fix:** The terminal error hook connected a crash directly to AI without copy-pasting
- **Exercise 10 — Generate a docstring:** `/doc` generated professional documentation in two keystrokes
- **Exercise 11 — Generate pytest cases:** Copilot Chat generated a full test suite from a plain-English description
- **Exercise 12 — Git commit via sparkle button:** The Source Control sparkle button generated a commit message by reading the staged diff
